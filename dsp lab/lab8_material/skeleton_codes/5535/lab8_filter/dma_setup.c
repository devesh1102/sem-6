...
void DMA1_setup(void) {
    ...
    DMA_IER = 0x00F0; /* Enable interrupts for DMA1CH0-DMA1CH3 */
    
	/* Step 5 - Setup sync event */
	register_value = DMA1_CESR1;
	DMA1_CESR1 = register_value | 0x0101; /* Set CH1, CH0 sync event to I2S2 transmit */
	register_value = DMA1_CESR2;
	DMA1_CESR2 = register_value | 0x0202; /* Set CH3, CH2 sync event to I2S2 receive*/

	/* Step 6 - Channel source addresses */
	DMA1_CH2_SSAL = 0x2A28;//I2S0_RXLT0; /* 16 LSBs of I2S2 left receive */
	DMA1_CH2_SSAU = 0x0000;
	
	DMA1_CH3_SSAL = 0x2A2C;//I2S0_RXRT0; /* 16 LSBs of I2S2 right receive */
	DMA1_CH3_SSAU = 0x0000;
	
	dma_address = convert_address(DMA_OutL);       /* convert address */
	DMA1_CH0_SSAL = (Uint16)dma_address;          /* keep LSBs */
	DMA1_CH0_SSAU = 0xFFFF & (dma_address >> 16); /* keep MSBs */
	
	dma_address = convert_address(DMA_OutR);       /* convert address */
	DMA1_CH1_SSAL = (Uint16)dma_address;
	DMA1_CH1_SSAU = 0xFFFF & (dma_address >> 16);
	
	/* Step 7 - Channel destination addresses*/
	dma_address = convert_address(DMA_InpL);
	DMA1_CH2_DSAL = (Uint16)dma_address;
	DMA1_CH2_DSAU = 0xFFFF & (dma_address >> 16);
	
	dma_address = convert_address(DMA_InpR);
	DMA1_CH3_DSAL = (Uint16)dma_address;
	DMA1_CH3_DSAU = 0xFFFF & (dma_address >> 16);
	
	DMA1_CH0_DSAL = 0x2A08;//I2S2_TXLT0; 0x2A08
	DMA1_CH0_DSAU = 0x0000;
	
	DMA1_CH1_DSAL = 0x2A0C;//I2S2_TXRT0;  0x2A0C
	DMA1_CH1_DSAU = 0x0000;
    ...
    DMA1_CH2_TCR2 = 0x3081;
    DMA1_CH3_TCR2 = 0x3081;
    DMA1_CH0_TCR2 = 0x3201;
    DMA1_CH1_TCR2 = 0x3201;
}
