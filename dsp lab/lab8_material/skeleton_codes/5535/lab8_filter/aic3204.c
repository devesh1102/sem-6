...
void AIC3204_Init(void)
{
    ...
    SYS_EXBUSSEL |= 0x0100;  // Configure Serial bus 0 for I2S2
...
void input_sample(short *l_chan, short *r_chan)
{
	/* Read Digital audio input */
        while((RcvR & I2S2_IR) == 0);  // Wait for receive interrupt to be pending
	*l_chan = I2S2_W0_MSW_R;
	*r_chan = I2S2_W1_MSW_R;
}

void output_sample(short l_chan, short r_chan)
{
	while((XmitR & I2S2_IR) == 0); // Wait for transmit interrupt to be pending
	I2S2_W0_MSW_W = l_chan;  // 16 bit left channel transmit audio data
    I2S2_W1_MSW_W = r_chan;  // 16 bit right channel transmit audio data
}
