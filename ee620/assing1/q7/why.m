...
for i = 1:samples
      ...
      delta_E(i) = abs(0.55 - (phi_f - phi_s(i)));
      dit(i) = dit_cal(delta_E(i));
      tit(i)= exp(-1*abs(0.55 - (phi_f - phi_s(i)))/(k*t/q))/(nii*vth*sigma);
      cit(i) = dit(i)*q;

%    dit(i)  = dit_cal(E(i));
%  
dit_avg =dit_avg+dit(i);
...
%tit = tit_avg /samples;
%tit = exp(-1*abs(phi_f)/kt_q)*10^3/(nii*vth*sigma);%%%%%
rit = tit ./cit;
gp_n = w*w*(q*(dit.*tit));
gp_d = 1 + ((w*tit).*(w*tit));
gp = gp_n./gp_d;
