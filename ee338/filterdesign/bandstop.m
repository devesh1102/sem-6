...
limitH = stop_lp/ d1^(1/(2*ns));
cutoff = 1.5;%%%my parameter between limitL - limitH
syms s
...
figure(3)%dont use it for analog
[h,f,w] = freqz(num,den,'whole',2001);
freqzplot(h,f,w);
title('Analog LOWPASS ')
figure(4)
...
figure(2)%dont use it for analog
% [h,f,w] = freqz(bpf_N1,bpf_D1,'whole',2001);
% freqzplot(h,f,w);
title('Analog Bandstop Transfer')
title('Analog Bandpass Transfer')
