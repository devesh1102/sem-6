...
N_min = ceil((A-8) / (2.285*2*(2*transition*pi)/(samp_rate)));        %empirical formula for N_min

%Window length for Kaiser Window
n=N_min+14;%why +12%tuneble parameter
%end
...
cutoff_1 = 2*pi*( wp2-(transition) )/samp_rate%tuneble parameter why?does????
cutoff_2 =2*pi*( wp1 + (transition) )/samp_rate%tuneble parameter
 ...
 FIR_Bandstop = bp_ideal .* kaiser_win;
 fvtool(FIR_Bandstop);         %frequency response

%magnitude response
[H,f] = freqz(FIR_Bandstop,1,1024, samp_rate);
figure(6)
plot(f,abs(H))
grid
